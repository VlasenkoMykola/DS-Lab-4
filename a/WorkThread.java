...
public class WorkThread extends Thread {
    ...
    private void findNameByPhone() {
        String phoneNumberForFind = "+380000000069";
        //while(true) {
            data.lockRead();
            System.out.println("Searching name by phone.");
            for (Account account : data.accounts) {
                if (account.getPhoneNumber().equals(phoneNumberForFind))
                    System.out.println("Founded in search name by phone " + account.toString());
            }
            try {Thread.sleep(5000);}
            catch (InterruptedException e) {e.printStackTrace();}
    ...
    private void RWAccounts(){
        Account a1 = new Account("Іван", "Іванов","Іванович","+380000000069");
        Account a2 = new Account("Лариса", "Аджубей","Трохимівна","+380000000000");
        boolean tt = false;
